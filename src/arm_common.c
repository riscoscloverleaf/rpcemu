...
// 2 lines below is for "exit_on_shutdown" option implementation
//#define SWI_OS_Reset	0x6A
//#define PowerDownMagic   0x46464F26
#define SWI_OS_ServiceCall	0x30
#define Service_ShutdownComplete 128
/**
...
realswi:
		...
		uint32_t r1 = arm.reg[1]; // make copy of R1

    	exception(SUPERVISOR, 0xc, 4);

        // Catch OS_ServiceCall Service_ShutdownComplete and exit from emulator
		// if configured to "exit RPCEmu on RISCOS shutdown"
		if (swinum == SWI_OS_ServiceCall && r1 == Service_ShutdownComplete && config.exit_on_shutdown) {
            rpcemu_emulator_exit();
        }
	}
