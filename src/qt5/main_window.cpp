...
static const uint8_t _left_alt_scan_codes[] = {0x11, 0};
static const uint8_t _pg_up_scan_codes[] = {0xe0, 0x7d, 0};
static const uint8_t _pg_down_scan_codes[] = {0xe0, 0x7a, 0};

void
MainWindow::wheelEvent(QWheelEvent * event) {
    if (event->delta() > 0) {
        keyboard_key_press(_left_alt_scan_codes);
        keyboard_key_press(_pg_up_scan_codes);
        keyboard_key_release(_pg_up_scan_codes);
        keyboard_key_release(_left_alt_scan_codes);
        //std::cerr << "Wheel up" << std::endl;
//        rpclog("Wheel up\n");
    } else {
        keyboard_key_press(_left_alt_scan_codes);
        keyboard_key_press(_pg_down_scan_codes);
        keyboard_key_release(_pg_down_scan_codes);
        keyboard_key_release(_left_alt_scan_codes);
        //std::cerr << "Wheel down" << std::endl;
//        rpclog("Wheel down\n");
    }
}
...
MainWindow::keyPressEvent(QKeyEvent *event)
{
	...
	switch (config.special_key) {
	    case RPCEMU_SPECAIL_KEY_CTRL_END:
            isSpecialKey = (Qt::Key_End == event->key() && (event->modifiers() & Qt::ControlModifier));
	        break;
        case RPCEMU_SPECAIL_KEY_CTRL_ALT_ENTER:
            isSpecialKey = ((Qt::Key_Enter == event->key() || Qt::Key_Return == event->key()) && (event->modifiers() & Qt::ControlModifier) && (event->modifiers() & Qt::ShiftModifier));
            break;
        case RPCEMU_SPECAIL_KEY_ALT_END:
            isSpecialKey = (Qt::Key_End == event->key() && (event->modifiers() & Qt::AltModifier));
            break;
        default:
            isSpecialKey = false;
            break;
...
void MainWindow::switch_to_fullscreen() {
...
//	connect(this, &MainWindow::main_display_signal, this, &MainWindow::main_display_update);
	...
	connect(this, &MainWindow::guest_clipboard_data_changed_signal, this, &MainWindow::guest_clipboard_data_changed);
    // connect host clipboard data changed signal
    connect(QGuiApplication::clipboard(), &QClipboard::dataChanged, this, &MainWindow::host_clipboard_data_changed);
...
/**
 * Signal received about clipboard data changed
 */
void MainWindow::host_clipboard_data_changed() {
    // ignore clipboard change if it comes from us
    QByteArray ba;
    QString txt = QGuiApplication::clipboard()->text(QClipboard::Clipboard);
    if (!txt.isEmpty()) {
        if (guest_clipboard_text != txt) {
            guest_clipboard_text = txt;
            emit this->emulator.clipboard_updated_on_host_signal(txt, ba);
            return;
        }
    }
    QImage image = QGuiApplication::clipboard()->image(QClipboard::Clipboard);
    if (!image.isNull()) {
        if (guest_clipboard_image != image) {
            guest_clipboard_image = image;
            QBuffer buffer(&ba);
...
/**
 * Signal received about guest (RiscOS) clipboard data changed
 */
void
MainWindow::guest_clipboard_data_changed(QString text, QImage img) {
    if (img.isNull()) {
        if (!text.isEmpty()) {
            guest_clipboard_text = text;
            QGuiApplication::clipboard()->setText(text);
        }
    } else {
//        fprintf(stderr,"guest_clipboard_data_changed image: %dx%d\n", img.width(), img.height());
        guest_clipboard_image = img;
