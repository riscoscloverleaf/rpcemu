	...
	model_copy(model_copy)
{
    ...
    start_fullscreen_checkbox = new QCheckBox("Start in full screen");
    exit_on_shutdown_checkbox = new QCheckBox("Exit RPCEmu on RISC OS Shutdown");
    startup_shutdown_vbox = new QVBoxLayout();
    startup_shutdown_vbox->addWidget(start_fullscreen_checkbox);
    startup_shutdown_vbox->addWidget(exit_on_shutdown_checkbox);
    ...
    special_key_3 = new QRadioButton("Alt-End");
    special_key_group = new QButtonGroup();
    special_key_group->addButton(special_key_1);
    special_key_group->addButton(special_key_2);
    special_key_group->addButton(special_key_3);
    special_key_vbox = new QVBoxLayout();
    special_key_vbox->addWidget(special_key_1);
    special_key_vbox->addWidget(special_key_2);
...
ConfigureDialog::dialog_accepted()
{
    ...
    if (start_fullscreen_checkbox->isChecked()) {
        new_config.start_fullscreen = 1;
    } else {
        new_config.start_fullscreen = 0;
    }

    if (exit_on_shutdown_checkbox->isChecked()) {
        new_config.exit_on_shutdown = 1;
    } else {
...
ConfigureDialog::applyConfig()
{
	...
	// Start fullscreen
    if(config_copy->start_fullscreen) {
        start_fullscreen_checkbox->setChecked(true);
    } else {
        start_fullscreen_checkbox->setChecked(false);
    }
    // Exit on shutdown
    if(config_copy->exit_on_shutdown) {
        exit_on_shutdown_checkbox->setChecked(true);
    } else {
    ...
    special_key_3->setChecked(false);
    switch(config_copy->special_key) {
        case   RPCEMU_SPECAIL_KEY_CTRL_END: special_key_1->setChecked(true);   break;
        case   RPCEMU_SPECAIL_KEY_CTRL_ALT_ENTER: special_key_2->setChecked(true);   break;
