...
config_load(Config * config)
{
    ...
    config->start_fullscreen = settings.value("start_fullscreen", "0").toInt();
    config->exit_on_shutdown = settings.value("exit_on_shutdown", "0").toInt();
    ...
    config->special_key = settings.value("special_key", "1").toInt();
...
config_save(Config *config)
{
    ...
    settings.setValue("special_key",   config->special_key);
    settings.setValue("sound_enabled",   config->soundenabled);
    settings.setValue("start_fullscreen",   config->start_fullscreen);
    settings.setValue("exit_on_shutdown",   config->exit_on_shutdown);
