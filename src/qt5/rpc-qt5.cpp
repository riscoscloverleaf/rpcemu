 ...
 */
void rpcemu_set_host_clipboard(int file_type, const char *data, unsigned int data_len) {
    switch (file_type) {
        case 0xfff: {
...
//                fprintf(stderr, "rpcemu_set_host_clipboard len:%d strlen:%d str:'%s'\n", data_len / 4, txt.length(), txt.toUtf8().data());
        ...
        case 0xc85: {
                QImage img;
                if (!img.loadFromData(reinterpret_cast<const uchar *>(data), data_len, "JPG")) {
#if defined __linux || defined __linux__
                    fprintf(stderr, "clipboard JPG img.loadFromData() failed\n");
#endif
        ...
        case 0xb60: {
                QImage img;
                if (!img.loadFromData(reinterpret_cast<const uchar *>(data), data_len, "PNG")) {
#if defined __linux || defined __linux__
                    fprintf(stderr, "clipboard PNG img.loadFromData() failed\n");
#endif
        ...
        default:
#if defined __linux || defined __linux__
            fprintf(stderr, "rpcemu_set_host_clipboard() unknown file type: %x\n", file_type);
#endif
            rpclog("rpcemu_set_host_clipboard() unknown file type: %x\n", file_type);
    }
}

/**
 * Helper function to call the emulator->exit() method on the
 * Emulator object from C.
 */
void
rpcemu_emulator_exit(void) {
    emulator->exit();
    QApplication::exit(0);
}
...
//	}
	...
	emu_thread->start();
    if (pconfig_copy->start_fullscreen) {
        main_window.switch_to_fullscreen();
    }
...
Emulator::Emulator()
{
	...
	connect(this, &Emulator::clipboard_updated_on_host_signal, this, &Emulator::clipboard_updated_on_host);
}
...
void Emulator::clipboard_updated_on_host(QString txt, QByteArray image) {
    if (!txt.isEmpty()) {
        ;
        QVector<uint32_t> ucs_str = txt.toUcs4();
        const char *data = (const char*)ucs_str.constData();
//        fprintf(stderr, "Emulator::clipboard_updated_on_host txt:'%s' len:%d\n", txt.toUtf8().data(), txt.length());
//        unsigned int ucschar;
//        for(int i = 0; i < txt.length(); i++) {
//            ucschar = ((unsigned int*)data)[i];
//            fprintf(stderr,"%08x %08x\n", ucschar, ucs_str[i]);
//        }

        clipboard_changed_on_host_notify(0xfff, data, txt.length() * 4);
        return;
    }

    if (!image.isNull()) {
        clipboard_changed_on_host_notify(0xc85, image.data(), image.length());
    }
}
